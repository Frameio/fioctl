...
DEFAULT_COLS = column_default(
    "projects", "id,team_id,name,owner_id,storage,collaborator_count,root_asset_id"
)
...
@click.option(
    "--columns",
    type=utils.ListType(),
    default=["id", "_type", "user.email", "deleted_at"],
)
...
@click.option("--format", type=utils.FormatType(), default="table")
@click.option(
    "--columns", type=utils.ListType(), default=["id", "_type", "email", "inserted_at"]
)
def collaborators(project_id, format, columns):
    collab_stream = fio.stream_endpoint(
        f"/projects/{project_id}/collaborators?sort=user.email"
    )
    pending_collab_stream = fio.stream_endpoint(
        f"/projects/{project_id}/pending_collaborators?sort=email"
    )
